function georgianpikersettime(date,selected){
  ...
  selected.next().find(".piker-pickedyear").text(date.format('YYYY'));
  selected.next().find(".piker-pickedday").text(date.format('dddd'));
  selected.next().find(".piker-pickeddate").text(date.format('D'));
  selected.next().find(".piker-pickedmonth").text(date.format('MMMM'));
  selected.next().find(".picker_nowdate").text(moment().format('YYYY/M/D'+' : امـروز'));
...
function georgianonclickfunc(date){
...
$('.picker-nextmonth').click(function(){
  var i = parseInt($(this).parents('.datepickerinput[typecalendar="georgian"]').index('.datepickerinput[typecalendar="georgian"]'));

georgianpikersettime(date.add(1, 'months'),$('.datepickerinput[typecalendar="georgian"]').eq(i));
})

$('.picker-prevmonth').click(function(){
  var i = parseInt($(this).parents('.datepicker').index('.datepicker'));
georgianpikersettime(date.subtract(1, 'months'),$('.datepickerinput[typecalendar="georgian"]').eq(i));
})
...
function georgianapendmonth(){
  var daysofweek = [0];
  for(i=0;i<=6;i++){
    daysofweek[i]= moment().weekday(i+1).format('ddd');
  }
...
function georgiandatepicker(){
  ...
  for(i = $('.datepicker').length; i >= 0; i--){
    pikersettime(date,$('.datepickerinput[typecalendar="georgian"]').eq(i));
  }
  georgianonclickfunc(date);
}
...
jQuery(document).ready(function () {
  if($('.datepickerinput[typecalendar]').attr('typecalendar') == "jalaali"){
    jalaalidatepicker();
  }else if($('.datepickerinput[typecalendar]').attr('typecalendar') == "georgian"){
    datepicker();
  }
});
